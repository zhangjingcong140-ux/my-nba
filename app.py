...
# ----------------- 2. 添加与删除 -----------------
    ...
    with tab1:
        with st.form("add_player_form"):
            ...
            position = st.selectbox("球员位置：", ["PG", "SG", "SF", "PF", "C"])
...
# ----------------- 3. 修改与交易 -----------------
    ...
    with tab1:
        ...
        if p_target:
            st.info(f"当前球员：**{p_target.name}** | 位置：**{p_target.position}** | 当前能力值：**{p_target.rating}**")
            ...
            with col_mod2:
                new_pos = st.selectbox("更新位置（可选）：", [p_target.position, "PG", "SG", "SF", "PF", "C"])
        ...
        if st.button("📥 导出传奇球员到 LegendPlayers.txt"):
            with open("LegendPlayers.txt", "w", encoding="utf-8") as f:
                for p in legend_players:
                    f.write(f"{p.name},{p.age},{p.team},{p.rating},{p.position}\n")
...
# ----------------- 5. 排序与展示 -----------------
    ...
    if sub_option == "能力值升序":
        ...
        if st.button("开始抽卡！"):
            ...
            st.balloons()
            st.success(f"🎉 抽中的球员是：**{chosen.name}** | 位置：[{chosen.position}] | 球队：{chosen.team} | 能力值：{chosen.rating}")
        ...
        if st.button("🌟 抽取精锐球员！"):
            if high_rating_pool:
                ...
                st.balloons()
                st.success(f"🔥 欧气爆发！抽中优质球员：**{chosen.name}** | 位置：[{chosen.position}] | 球队：{chosen.team} | 能力值：**{chosen.rating}**")
            else:
...
# ----------------- 6. 🏀 5v5 斗牛对决 -----------------
    ...
    if len(players) < 10:
        ...
        player_dict = {f"{p.name} [{p.position}] ({p.team} - {p.rating}分)": p for p in players}
        ...
        if battle_mode == "🔥 盲盒抽卡 5v5":
            ...
            if len(auc_blue) < 5 or len(auc_red) < 5:
                ...
                if both_money_empty or not high_rating_pool:
                    ...
                    if target:
                        st.info(f"🌟 **当前竞拍球员：** **{target.name}** （位置：[{target.position}] | 球队：{target.team} | 能力值：**{target.rating}**）")
                        ...
                        with st.form("auction_form"):
                            ...
                            if submit_bid:
                                ...
                                if blue_bid == 0 and red_bid == 0:
                                    ...
                                    st.session_state.auction_logs.append(f"🔵 蓝方以 **${blue_bid}** 竞拍成功获得 **{target.name}** [{target.position}] ({target.rating}分)")
                                    st.session_state.current_target_player = None
                                    reset_match_state()
                                    st.rerun()
                                    ...
                                    st.session_state.auction_logs.append(f"🔴 红方以 **${red_bid}** 竞拍成功获得 **{target.name}** [{target.position}] ({target.rating}分)")
                                    st.session_state.current_target_player = None
                                    reset_match_state()
                                    st.rerun()
                                else:
                                    ...
                                    if winner == "blue" and not blue_full:
                                        st.session_state.blue_money -= blue_bid
                                        st.session_state.auction_blue_team.append(target)
                                        st.session_state.auction_logs.append(f"🎲 双方出价相同，系统抽签：🔵 蓝方获得 **{target.name}** [{target.position}]")
                                    else:
                                        st.session_state.red_money -= red_bid
                                        st.session_state.auction_red_team.append(target)
                                        st.session_state.auction_logs.append(f"🎲 双方出价相同，系统抽签：🔴 红方获得 **{target.name}** [{target.position}]")
                                    st.session_state.current_target_player = None
                                    reset_match_state()
                                    st.rerun()
        ...
        if len(blue_team) == 5 and len(red_team) == 5:
            ...
            calc_blue_team = [Player(p.name, p.age, p.team, p.rating, p.position) for p in blue_team]
            calc_red_team = [Player(p.name, p.age, p.team, p.rating, p.position) for p in red_team]
